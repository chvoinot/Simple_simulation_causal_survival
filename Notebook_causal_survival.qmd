...
\begin{aligned}
    ...
    &= \mathbb{E}[T \wedge \tau|A=1] - \mathbb{E}[T \wedge \tau|A=0]  && \tiny\text{(First possible estimator)}\\
    &= \int_{0}^{\tau}{\mathbb{P}(T>t|A=1) - \mathbb{P}(T>t|A=0) dt} \\
    &= \int_{0}^{\tau}{S(t|A=1)-S(t|A=0)dt} && \tiny\text{(Second possible estimator)}\\
\end{aligned}
...
### FAUX : In using the function min

The RMST can be estimated using the function min as follows: $$
    \theta_{RMST}= \mathbb{E}[T \wedge \tau|A=1] - \mathbb{E}[T \wedge \tau|A=0] 
$$

**Implementation**

FAUX : ne prend pas en compte la censure

```{r}
#Function to estimate the RMST using the function min
RMST_min <- function(data, status=data$status, T_obs=data$T_obs, tau){
  data$rmst<- pmin(data$T_obs, tau)
  data_treated <- data %>%
    dplyr::filter(A==1)
  data_control <- data %>%
    dplyr::filter(A==0)
  #Compute the RMST for the treated group
  rmst_treated <- mean(data_treated$rmst)
  #Compute the RMST for the control group
  rmst_control <- mean(data_control$rmst)
  #Compute the RMST
  rmst <-rmst_treated - rmst_control
  return(rmst)
  }
...
# kaplan meier estimator with weight (matrix (nb ind x Y.grid))
Kaplan_meier_handmade_w <- function(data,status=data$status,T_obs=data$T_obs,weights= NULL,Y.grid=NULL){
  n <- length(T_obs)
  if (is.null(Y.grid)){
    Y.grid <- sort(unique(T_obs))
  }
  ...
  # When SC is calculated in using non-parametric model : SC is not based on cov so not individual dependent 
  if (length(weights)==n){
    # the weight is the same for all individual
    weights <- matrix(rep(weights,times = length(Y.grid)), ncol =  length(Y.grid))
  }
  d <- rep(NA,length(Y.grid))
  n <- rep(NA,length(Y.grid))
  S <- rep(NA,length(Y.grid))
  w <- rep(NA,length(Y.grid))
  for (i in 1:length(Y.grid)){
    d[i] <- sum((T_obs==Y.grid[i] & status ==1))#*weights[,i])
    n[i] <- sum((T_obs >= Y.grid[i]))#*weights[,i])
    w[i] <- sum(weights[,i])
  }
  S <- cumprod(1-d/n)
  df<-data.frame(d=d,n=n,S= S, T = Y.grid,w=w)
  return(df)
}


#test <- Kaplan_meier_handmade_w(DATA_treated,T_obs=DATA_treated$T_obs,status=DATA_treated$status,Y.grid=Y.grid, weights = weights_treated)
#test2 <- Kaplan_meier_handmade_w(data,status=data$status,T_obs=data$T_obs,weights=weights,Y.grid = Y.grid)
# test <- Kaplan_meier_handmade_w(data,status=data$status,T_obs=data$T_obs,weights=NULL,Y.grid = Y.grid)
# test3 <- Kaplan_meier_handmade(data,status=data$status,T_obs=data$T_obs)
# 
# plot(test$T,test$S)
# lines(test2$T,test2$S,col=2)
# lines(test3$T,test3$S,col=3)
...
IPCW_Kaplan_meier <- function(data,tau){
  ...
  S1 <- Kaplan_meier_handmade_w(DATA_treated,T_obs=DATA_treated$T_obs_tau,status=DATA_treated$status_tau, weights = weights_treated)
  S0 <- Kaplan_meier_handmade_w(DATA_not_treated,T_obs=DATA_not_treated$T_obs_tau,status=DATA_not_treated$status_tau,weights = weights_not_treated)
  
  #S1bis <- Kaplan_meier_handmade_w(DATA_treated,T_obs=DATA_treated$T_obs_tau,status=DATA_treated$status_tau, weights = weights_treated)
  #S0bis <- Kaplan_meier_handmade_w(DATA_not_treated,T_obs=DATA_not_treated$T_obs_tau,status=DATA_not_treated$status_tau,weights = weights_not_treated)
  
  #intA1 <- trapezoidal_integration(S1bis$T, S1bis$S)
  #intA0 <- trapezoidal_integration(S0bis$T, S0bis$S)

  #RMSTbis <- intA1 - intA0
  
  #Exact same results : 
  RMST <- RMST(S1,S0,tau=20)
...
IPCW_min <- function(data,tau){
  ...
  #on selectionne les poids correspondant à l'ind i et au temps T_obs_tau
  #weights_treated <- weights_treated[cbind(1:nrow(DATA_treated),match(DATA_treated$T_obs_tau,Y.grid))]
  
  DATA_treated$weights <- weights_treated[cbind(1:nrow(DATA_treated),match(DATA_treated$T_obs_tau,Y.grid))]
  #DATA_treated$weights <- weights_treated[cbind(1:nrow(DATA_treated),match(DATA_treated$T_obs_tau,Y.grid))]
  DATA_not_treated$weights <- weights_not_treated[cbind(1:nrow(DATA_not_treated),match(DATA_not_treated$T_obs_tau,Y.grid))]
  
  DATA_treated$RST <- DATA_treated$T_obs_tau*DATA_treated$weights*DATA_treated$status_tau
  DATA_not_treated$RST <- DATA_not_treated$T_obs_tau*DATA_not_treated$weights*DATA_not_treated$status_tau
...
\begin{aligned}
    ...
    & =\mathbb{E}\left[\mathbb{E}[(T \wedge \tau) \mid A, X]\left(\frac{A}{e(X)}-\frac{1-A}{1-e(X)}\right)\right] \tiny\text { (By consistency) }\\
     & =\mathbb{E}\left[(\widetilde{T} \wedge \tau)\left(\frac{A}{e(X)}-\frac{1-A}{1-e(X)}\right)\right]\\
\end{aligned}
$$ {#eq-RMST-IPTW}
...
IPTW_Kaplan_Meier<- function(data,e_hat,tau){
  data$T_obs_tau <- pmin(data$T_obs, tau)
  data$status_tau <- as.numeric((data$T_obs>=tau) | (data$T_obs<tau &  data$status == 1 ))
  W <- (data$A == 1) * (1/e_hat) + (data$A==0) * (1)/(1-e_hat)
  fitipw <- survfit(Surv(T_obs_tau, status_tau) ~ A, data = data,weights =W )
  # Summarize
  resipw.sum <- surv_summary(fitipw, data = data)
  res_ipw <- attr(resipw.sum, "table")
  
  
  mean_ipw <- (res_ipw$rmean[2] - res_ipw$rmean[1])
  return(list("intA0"=res_ipw$rmean[1],"intA1"=res_ipw$rmean[2],"RMST"=mean_ipw))
}

#e_hat <- estimate_propensity_score(data,treatment_name="A",type_of_model="reglog")
...
```{r}
IPTW_IPCW_Kaplan_Meier<- function(data,e_hat,tau){
  data$T_obs_tau <- pmin(data$T_obs, tau)
  data$status_tau <- as.numeric((data$T_obs>=tau) | (data$T_obs<tau &  data$status == 1 ))
  data$censor.status_tau <- 1-data$status_tau
  
  S_C_hat <-estimate_survival_function(data,c("X1","X2","X3"),Y.grid,T_obs="T_obs_tau",status="censor.status_tau")
  S_C <-  S_C_hat$S_hat
  status_tau_t <- sapply(Y.grid,function(t) as.numeric((data$T_obs>t) | (data$T_obs<=t &  data$status == 1 )))
  weights <- status_tau_t/S_C*(data$A/e_hat + (1-data$A)/(1-e_hat))
...
simulate_data_RCT <- function(n,mu= c(1, 1, -1, 1), sigma=diag(4),tau,coefT0=0.1,
parsS=c(1,1,-1,1),coefC=0.03,parsC=c(0.1,0.1,-0.2,-0.2),scenario="RCT2"){
  if (scenario=="RCT1"){
...
data_obs1 <- simulate_data_obs(n=2000,tau=50,coefC=0.05,scenario="Obs1")
...
The handmade Kaplan meier estimator is the same as the survfit function
of the survival package.

The results of the four methods are presented in the section @sec-KM are
displayed below :
...
tau <- seq(0,48,by=1)
res1_2 <- sapply(tau,function(x) RMST_1_2(data,tau=x))
res3 <- sapply(tau,function(x) RMST_3(data,tau=x))
res4 <- sapply(tau,function(x) RMST_4(data,tau=x))
res4bis <- sapply(tau,function(x) RMST_4(data_no_cens,tau=x))
...
# plot des 2 calculs de RMST en fonction de tau : 
tau <- seq(0,50,by=1)
res5 <- sapply(tau,function(x) IPCW_Kaplan_meier(data,tau=x))
res5bis <- sapply(tau,function(x) IPCW_Kaplan_meier(data_no_cens,tau=x))

matplot(tau,res5[1,],type="l",lty=1,col=1,ylab="RMST",xlab="tau")+
  #lines(tau,res5[4,],lty=4,col=3)+
  lines(tau,res5bis[1,],lty=2,col=2)+
  abline(h=4,xlim=c(0,50),col="black",lty=2)
...
e_hatbis <- estimate_propensity_score(data_no_cens,c("X1","X2","X3","X4"),type_of_model="reglog")
res6 <- sapply(tau,function(x) Adjusted_Kaplan_Meier(data,e_hat=e_hat,tau=x))
res6bis <- sapply(tau,function(x) Adjusted_Kaplan_Meier(data_no_cens,e_hat=e_hatbis,tau=x))


matplot(tau,res6[3,],type="l",lty=1,col=1,ylab="RMST",xlab="tau",ylim=c(0,6))+ 
  lines(tau,res6bis[3,],lty=2,col=2)+
  abline(h=4,xlim=c(0,50),col="black",lty=2)
...
all_estimates <- function(data,sample.size,tau){
  ...
  ATE_km_rct <- RMST(S_A1=km_rct1,S_A0=km_rct0,tau=tau)
  
  ATE_pack <- rmst2(data$T_obs, data$status, arm=data$A, tau = tau)[[5]][1]
  
  ATE_km_adj <- Adjusted_Kaplan_Meier(data,e_hat,tau=tau)
  
  ATE_g_formula <- g_formula_cox_T_learner(data,c("X1","X2","X3","X4"),Y.grid=c(1:tau))
  
  ATE_IPCW <- IPCW_Kaplan_meier(data, tau=tau)  

  data.frame <- data.frame("sample.size" = rep(sample.size, 6),
                               "estimate" = c(ATE_naive, ATE_km_rct$RMST,
                                              ATE_km_adj$RMST,ATE_IPCW$RMST,  ATE_g_formula,ATE_pack),
                               "estimator" = c("Naive", "KM", "IPTW Adjusted KM", "IPCW Adjusted KM", "G_formula cox","Package"),
                               "nuisance" = rep("linear + cox", 6))
...
```{r,eval=FALSE}
# rct1
simulation_rct1<-compute_estimator(data_rct1,n_sim,tau=10)
