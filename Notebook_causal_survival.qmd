...
IPCW_Kaplan_meier <- function(data,Y.grid= sort(unique(data$T_obs)),tau){
  data$T_obs_tau <- ifelse(data$T_obs>=tau,tau,data$T_obs)
  ...
  S1 <- Kaplan_meier_handmade_w(DATA_treated,T_obs=DATA_treated$T_obs,status=DATA_treated$status, weights = weights_treated)
  S0 <- Kaplan_meier_handmade_w(DATA_not_treated,T_obs=DATA_not_treated$T_obs,status=DATA_not_treated$status,weights = weights_not_treated)
  ...
  return(list(RMST = RMST$RMST, ATE_treated = RMST$intA1, ATE_not_treated = RMST$intA0, RMSTbis=RMSTbis, ATE_treatedbis = intA1, ATE_not_treatedbis = intA0))
}

#IPCW_Kaplan_meier(data,tau=35)
...
```{r}
Adjusted_Kaplan_Meier<- function(data,e_hat,tau){
...
We conducted two simples simulations to simulate RCTs
studies, baseline covariates with no time dependency. The first simulation represents a scenario with
independent censoring and the second one with conditional dependent
censoring.
...
parsS=c(0.5,-0.1,0.3,0.2),coefC=0.03,parsC=c(0.1,0.1,-0.2,-0.2),scenario="RCT2"){
  if (scenario=="RCT1"){
...
```{r}
data_rct1 <- simulate_data_RCT(n=2000,tau=5,scenario="RCT1")
...
We conducted two simples simulations to simulate observationnal
studies with a static treatment assignment, baseline covariates with no
time dependency. The first simulation represents a scenario with
independent censoring and the second one with conditional dependent
censoring.
...
parsS=c(0.5,-0.1,0.3,0.2),coefC=0.03,parsC=c(0.1,0.1,-0.2,-0.2),scenario="Obs2"){
  if (scenario =="Obs1"){
    # Generate X from multivariate normal distribution
    X <- MASS::mvrnorm(n, mu, sigma)
    X <- as.data.frame(X)
...
data <- simulate_data_obs(n=2000,tau=50,scenario="Obs2")
head(data,10)

#data with no informative censoring
data_no_cens <- simulate_data_obs(n=2000,tau=50,scenario=2,coefC=0.05,scenario="Obs1")
head(data_no_cens,10)
```
